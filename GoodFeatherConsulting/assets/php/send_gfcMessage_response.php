...
/* Reset your password form, sends reset.php password link */
    ...
    $gfcMessage_email = $mysqli->escape_string($_POST['gfcMessage_email']);
    $gfcMessage_response = $mysqli->escape_string($_POST['gfcMessage_response']);
    $responder = $_SESSION['userFullName'];

    //Update response date in gfcMessages table
    $statement = $mysqli->prepare("UPDATE gfcMessages SET responder=?, responseDate=? WHERE id=?"); 
    mysqli_stmt_bind_param($statement, "sss", $responder, $current_time, $gfcMessage_id);
    if (mysqli_stmt_execute($statement)){
        //create generate the email
        $to = $gfcMessage_email;
        $subject = 'Good Feather Consulting | Message Response';
        $headers = "From: ".$_SESSION['userEmail']."\r\n";
        $message_body = $gfcMessage_response;

        if(mail($to, $subject, $message_body, $headers)){
            $_SESSION['message'] = "Response Delivered, email thread created between ". $gfcMessage_email ." and " . $_SESSION['userEmail'];
