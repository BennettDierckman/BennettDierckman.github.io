<?php
    ...
    $statement = $mysqli->prepare("SELECT * FROM callRequests WHERE id=?"); 
    mysqli_stmt_bind_param($statement, "s", $callRequest_id);
    ...
    if ($count == 1){
        ...
        mysqli_stmt_bind_result($statement, $result_id, $result_name, $result_phone, $result_email, $result_requestDate, $result_responder, $result_responseDate, $result_callDate, $result_callAnswered);
        while(mysqli_stmt_fetch($statement)){
          ...
          $callDate = $result_callDate;
          $callAnswered = $result_callAnswered;
        }
    }else{
      //Message does not exist in gfcMessages table (something went wrong)
      $_SESSION['message'] = "Unable to locate request in database."; //set error message
...
<html lang="en-US">
    <head>
        ...
        <!--    Document Title-->
        <!-- =============================================-->
        <title>GFC | Call Answered?</title> <!-- Shows in browser tab -->
        <!--  -->
        <!--    Favicons-->
        <!--    =============================================-->
                    ...
                    <div class="row h-full align-items-center">
                        <div class="col-lg-10 mx-auto" data-zanim-timeline="{}" data-zanim-trigger="scroll">
                            <div class="background-white radius-secondary p-4 p-md-5 mt-0" data-zanim='{"delay":0.1}'>
                                ...
                                <hr>
                                <h3 class="fs-2 mb-3">Responding to Call Request</h3> 
                                <div class="row text-left">
                                  ...
                                  <div class="col-4">
                                    <h5 class="mb-0">Call Date:</h5>
                                    <p><?php echo $callDate; ?></p>
                                  </div>
                                  <div class="col-10 mx-auto">
                                    <h5 class="mb-0">Call Answered:</h5>
                                    <form method="POST" action="send_callRequest_answer.php">
                                      <textarea class="form-control background-white mb-3" rows="1" name="callRequest_answer" id="callRequest_answer" placeholder="Yes/No" required></textarea>
                                      <div class="row">
                                        <div class="col-6">
                                          <a href="mainDashboard.php" class="btn btn-danger btn-block w-100">Cancel</a>
                                        </div>
                                        <div class="col-6">
                                          <?php echo '<input type="hidden" name="callRequest_id" value="'.$id.'">' ?>
                                          <?php echo '<input type="hidden" name="callRequest_email" value="'.$email.'">' ?>
