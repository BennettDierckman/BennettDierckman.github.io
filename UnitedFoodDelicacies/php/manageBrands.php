    ...
    <div class="crat">
      ...
      <div class="row">
        <div class="col-12 mx-auto" style="background-color: #F2f3f4;">
          <h3 class="text-center mt-4 mb-2">Existing Brands</h3>
          <?php
            ...
            if ($numRows > 0){ //They have pending attendance
              $counter = 1;
              echo '<span style="display: flex; justify-content: center;"><table class="table-custom mx-auto" border=1 color:white><tr><th>ID #</th><th>Brand Name</th><th>Edit Button</th></tr>';
              while($row = $result->fetch_assoc()){
                ...
                echo "<td>
                        <div class='edit-delete-btns'>
                          <form action='alterBrandInfo.php' method='POST'>
                            <input type='hidden' name='brandId' value='".$row['id']."'>
                            <button type='submit'  style='font-size: 20px;'>Edit</button>
                          </form>
                          <form action='deleteBrand.php' method='POST' onsubmit=\"return confirm('Are you sure you want to delete the brand ".$row["brand"]."?');\">
                            <input type='hidden' name='brandId' value='".$row['id']."'>
              ...
              echo "</table></span>";
              echo "Total Number of Brands: ".($counter-1).'<br>';
            }
            else { //no pending attendance
              echo "<p style='margin-bottom: 20px;'>No existing brands.</p>";
            }
          ?>
          <h3 class="text-center mt-4 mb-2">Register a new Brand Name</h3>
          <form method="post" enctype="multipart/form-data" action="registerBrand.php">
            <div class="form-group mb-4 mx-auto" style="max-width:1000px;">
              <input type="text" class="form-control" placeholder="Brand name" style="max-width: 350px; margin-left:auto; margin-right:auto;" id="brandName" name="brandName" required>
            </div>
