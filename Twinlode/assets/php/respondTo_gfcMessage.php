<?php
    ...
    $gfcMessage_id = $mysqli->escape_string($_POST["gfcMessage_id"]);

    //get the selected general message's information and bring it into scope
    $statement = $mysqli->prepare("SELECT * FROM gfcMessages WHERE id=?"); 
    mysqli_stmt_bind_param($statement, "s", $gfcMessage_id);
    ...
    if ($count == 1){
        ...
        mysqli_stmt_bind_result($statement, $result_id, $result_name, $result_email, $result_company, $result_title, $result_message, $result_messageDate, $result_responder, $result_responseDate);
...
<html lang="en-US">
    <head>
        ...
        <!--    Document Title-->
        <!-- =============================================-->
        <title>GFC | General Message</title> <!-- Shows in browser tab -->
        <!--  -->
        <!--    Favicons-->
        <!--    =============================================-->
                    ...
                    <div class="row h-full align-items-center">
                        <div class="col-lg-10 mx-auto" data-zanim-timeline="{}" data-zanim-trigger="scroll">
                            <div class="background-white radius-secondary p-4 p-md-5 mt-0" data-zanim='{"delay":0.1}'>
                                ...
                                <hr>
                                <h3 class="fs-2 mb-3">Responding to General Message</h3> 
                                <div class="row text-left">
                                  ...
                                  <div class="col-10 mx-auto">
                                    <h5>Response:</h5>
                                    <form method="POST" action="send_gfcMessage_response.php">
                                      <textarea class="form-control background-white mb-3" rows="7" name="gfcMessage_response" id="gfcMessage_response" placeholder="Your Message" required></textarea>
                                      <div class="row">
                                        <div class="col-6">
                                          <a href="mainDashboard.php" class="btn btn-danger btn-block w-100">Cancel</a>
                                        </div>
                                        <div class="col-6">
                                          <?php echo '<input type="hidden" name="gfcMessage_id" value="'.$id.'">' ?>
                                          <?php echo '<input type="hidden" name="gfcMessage_email" value="'.$email.'">' ?>
